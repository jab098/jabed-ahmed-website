...
it('ends with the approved contact actions and an interactive signal footer', () => {
  ...
  expect(container.querySelector('#contact')).toHaveAttribute('data-scroll-waypoint', 'contact')

  const footer = screen.getByRole('contentinfo')
  const colour = screen.getByRole('button', { name: 'Change colour' })
  expect(footer).toHaveAttribute('data-scroll-waypoint', 'footer')
  expect(footer).toHaveAttribute('data-scroll-align', 'viewport')
